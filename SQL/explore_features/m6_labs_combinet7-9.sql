# use the latest cohort available after processing vital signs
...
(
    lab_name in 
        (
--             "Glucose, Non-fasting", "Glucose", "Base Excess Arterial for POC", "Bicarbonate, Art for POC", "Hct (Est)", "HCT, POC", "Hematocrit (Manual Entry) See EMR for details", "Hgb, calculated, POC", "HgB", "Potassium, whole blood, ePOC", "Potassium", "Lactic Acid", "Oxygen Saturation for POC", "Arterial pCO2 for POC", "pH by Meter", "Arterial pH for POC", "Platelets", "Arterial pO2 for POC", "Total Bilirubin", "TCO2, (ISTAT)", "CO2 Arterial Total for POC", "Troponin I, POCT", "WBC count", -- additional lab_names, included in base_name (checked - could be wrong)
        ...
        'Neut, ABS (Seg+Band) (man diff)''Basophils, ABS (man diff)''Eosinophils, ABS (man diff)''Monocytes, ABS (man diff)''Lymphocytes, ABS (man diff)''Lymphocytes, Abs.''Neutrophils, Absolute (Manual Diff)''Mono, ABS (man diff)''Lym, ABS (man diff)''eGFR''Baso, ABS (man diff)''Eos, ABS (man diff)') -- from finalized lab names of Triage, though basename is in the below list, it didn't appear in lab name
    or base_name in 
        ('AG', 'AGAP', 'ALB', 'ALT', 'AST', 'ALKP', 'BASOAB', 'BE', 'BUN', 'CA', 'CAION',  'CL','CO2','CR', 'EOSAB', 'GLOB', 'GLU', 'GLUURN', 'HCO3A', 'HCO3V', 'HCT', 'HGB', 'INR', 'K', 'LAC', 'LACWBL', 'LYMAB', 'MG', 'MONOAB', 'NA', 'NEUTAB', 'O2SATA', 'O2SATV', 'PCAGP', 'PCBUN', 'PCCL', 'PCO2A', 'PCO2V', 'PH', 'PHA', 'PHCAI', 'PHV', 'PLT', 'PO2A', 'PO2V', 'PT', 'RBC', 'TBIL', 'TCO2A', 'TCO2V', 'TNI', 'TP', 'WBC', 'XLEUKEST', 'XUKET', 'UGLU', 'NEUTABS', 'EGFR')
) 
